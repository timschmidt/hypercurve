...
impl RetainedCornerExtensionCarrier2<'_> {
    ...
    fn retained_endpoint(self, previous: bool) -> BezierParameter2 {
        match self {
            Self::Curve(_) => {
                BezierParameter2::Exact(if previous { Real::zero() } else { Real::one() })
            }
...
impl CurveRegion2 {
    ...
    fn retained_chord_fillet_fragments(
        ...
        chord_cut.point = match fillet_parameter
            .coincident_point_evidence(&terminal_circle, policy)
            .map_err(|cause| curve_region_edit_error(CurveOperation2::Fillet, cause))?
        {
            Classification::Decided(Some(point)) => point,
    ...
    fn retained_fillet_fragments(
...

        {
            ...
            let fillet_parameter = match other_fragment {
                ...
                BezierSplitFragment2::AlgebraicChord(_) => {
                    unreachable!("chord companions returned through the selected-frame branch")
                }
    ...
    fn canonicalize_retained_corner_cut(
        ...
        let mut retained_endpoint = CornerTrimCut2 {
            parameter: CurveRegionParameter2::from_bezier(carrier.retained_endpoint(previous)),
