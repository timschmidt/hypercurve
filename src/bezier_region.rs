...
use crate::curve::{
    CornerTrimCut2, CornerTrimParameter2, ExactCornerCarrier2, exact_corner_carrier,
    solve_exact_chamfer_corner, solve_exact_fillet_corner, try_map_corner_solutions,
...
struct ExactOffsetSpan2 {
    fragments: Vec<BezierSplitFragment2>,
    source_start: RationalBezierIntersectionPointEvidence2,
    ...
    start_tangent: Option<(Real, Real)>,
    end_tangent: Option<(Real, Real)>,
}
...
fn exact_offset_span_from_materialized_curve(
    ...
    Ok(Classification::Decided(ExactOffsetSpan2 {
        fragments,
        source_start: curve.start().clone().into(),
...
fn exact_offset_span_from_native_arc(
    ...
    match real_sign(&radius_scale, policy) {
        Some(RealSign::Zero) => {
            ...
            Ok(Classification::Decided(ExactOffsetSpan2 {
                fragments: Vec::new(),
                source_start: source.start().clone().into(),
...
fn exact_algebraic_cusp_semicircle_endpoint(
    ...
    match fragment.endpoint_point_image(at_start, policy)? {
        Some(point) => Ok(Classification::Decided(
...
fn exact_offset_span_from_algebraic_cusp_semicircle(
    ...
    Ok(Classification::Decided(ExactOffsetSpan2 {
        ...
        offset_start,
        offset_end,
        // Algebraic tangent images are unnecessary at a smooth retained
        // junction: shared endpoint evidence returns before the corner path.
        // A genuinely new corner still reports Unsupported instead of using
        // a rounded tangent.
        start_tangent: None,
        end_tangent: None,
    }))
...
fn append_exact_offset_join(
    ...
    match previous.source_end.same_point(&next.source_start, policy) {
        Classification::Decided(true) => {}
        Classification::Decided(false) => return Err(CurveError::DisconnectedCurvePath),
        Classification::Uncertain(reason) => {
            return Ok(Classification::Uncertain(reason));
    ...
    match style {
        ...
        OffsetCornerStyle2::Bevel if !inward => {
            ...
            append_exact_line_join(fragments, from, to, policy)
        }
...
fn append_exact_miter_join(
    ...
    let Some(RealSign::Positive | RealSign::Negative) = denominator_sign else {
        return match denominator_sign {
            Some(RealSign::Zero) => {
                append_exact_line_join(fragments, previous_offset_end, next_offset_start, policy)
            }
            None => Ok(Classification::Uncertain(UncertaintyReason::RealSign)),
    ...
    match append_exact_line_join(fragments, previous_offset_end, &miter, policy)? {
        Classification::Decided(()) => {}
        Classification::Uncertain(reason) => return Ok(Classification::Uncertain(reason)),
    }
    append_exact_line_join(fragments, &miter, next_offset_start, policy)
}
...
impl CurveRegion2 {
    ...
    fn chamfer_loop_vertex_by_setbacks_raw(
        ...
        if mode == CurveCornerMode2::TrimOnly {
            ...
            if let Some(solutions) = self.retained_materialized_chamfer_solutions(
                loop_index,
                vertex_index,
...
fn classify_algebraic_point_against_retained_loop(
    ...
    if boundary_loop.fragments().iter().any(|fragment| {
        matches!(
            fragment,
            BezierSplitFragment2::AlgebraicChord(_)
                | BezierSplitFragment2::AlgebraicCuspSemicircle(_)
    ...
    let mut fragments = Vec::with_capacity(boundary_loop.fragments().len());
    for fragment in boundary_loop.fragments() {
        match retained_fragment_algebraic_ray_curve(fragment, policy)? {
            Classification::Decided(fragment) => fragments.push(fragment),
            Classification::Uncertain(reason) => {
                return Ok(Classification::Uncertain(reason));
            }
        }
    }
    let (direction_x, direction_y) = match algebraic_ray_direction(&fragments, point, policy)? {
        Classification::Decided(Some(direction)) => direction,
        Classification::Decided(None) => {
            return Ok(Classification::Decided(ContourPointLocation::Boundary));
        }
    ...
    for fragment in &fragments {
        ...
        let delta = match algebraic_point_rational_curve_ray_winding(
            fragment,
            point,
            &direction_x,
            &direction_y,
...
#[cfg(feature = "predicates")]
fn algebraic_ray_direction(
    fragments: &[AlgebraicRayRationalFragment2],
    point: &RationalBezierAlgebraicPointPredicate2<'_>,
    policy: &CurveContext,
    ...
    for slope in 0..candidate_count {
        ...
        if admissible {
            return Ok(Classification::Decided(Some((direction_x, direction_y))));
        }
    }
    Ok(Classification::Uncertain(last_reason))
}
