...
#[derive(Debug)]
enum BezierAlgebraicCuspFiberProjection2 {
...
///
/// The analytic source range remains in ascending Bezier-parameter order.
...
/// the pair-shared parallel parameter authority. `orientation` compares
/// increasing parameters on the two supporting carriers, independently of
/// either boundary fragment's traversal bit.
#[derive(Clone, Debug)]
#[allow(dead_code)]
pub(crate) struct BezierAlgebraicCuspSemicircleParallelOverlap2 {
    parallel_range: BezierParameterRange2,
...
impl PartialEq for BezierAlgebraicCuspSemicircleParallelOverlap2 {
    fn eq(&self, other: &Self) -> bool {
        self.parallel_range == other.parallel_range
...
#[allow(dead_code)]
impl BezierAlgebraicCuspSemicircleParallelOverlap2 {
    pub(crate) const fn parallel_range(&self) -> &BezierParameterRange2 {
        &self.parallel_range
    }
...
#[derive(Debug)]
#[allow(dead_code)]
struct BezierAlgebraicCuspSemicirclePairSystem2 {
    /// The second selected cusp's defining polynomial, used as exact
    /// independent-root incidence evidence for every pair predicate.
    incidence: BivariatePolynomial,
    /// Squared center separation over the shared squared denominator.
    center_distance_squared: BivariatePolynomial,
    /// Circle-circle discriminant. Positive yields two support contacts, zero
    /// one tangent contact, and negative no support contact.
    discriminant: BivariatePolynomial,
    /// Selected-half predicates whose radical coefficient is for branch `+1`.
    first_selected_half_plane: BezierAlgebraicCuspTwoTermExpression2,
    second_selected_half_plane: BezierAlgebraicCuspTwoTermExpression2,
    /// Monotone diameter predicates whose radical coefficient is for branch `+1`.
    first_diameter_side: BezierAlgebraicCuspTwoTermExpression2,
    second_diameter_side: BezierAlgebraicCuspTwoTermExpression2,
    /// Radius squared multiplied by the common positive denominator of the
    /// corresponding diameter expression.
...
#[derive(Debug)]
#[allow(dead_code)]
struct BezierAlgebraicCuspSemicircleCoincidentSystem2 {
    incidence: BivariatePolynomial,
    /// `dot(R1,R2)` over the shared positive squared denominator.
    radial_dot: BivariatePolynomial,
    /// `cross(R1,R2)` over the same positive squared denominator.
    radial_cross: BivariatePolynomial,
    first_radius_squared_denominator: BivariatePolynomial,
    second_radius_squared_denominator: BivariatePolynomial,
}
...
impl BezierAlgebraicCuspSemicircle2 {
    ...
    pub(crate) fn point_at(
        ...
        let normal_scale = &denominator * self.data.frame.data.parallel.distance()
        ...
        Ok(Classification::Decided(
            self.data.frame.point_image_from_frame_scales(
                &denominator,
                &normal_scale,
                &tangent_scale,
                policy,
    ...
    pub(crate) fn tangent_at(
        ...
        Ok(Classification::Decided(
            self.data.frame.tangent_image_from_frame_scales(
    ...
    pub(crate) fn start_point_image(
        ...
        let distance = self.data.frame.data.parallel.distance() + &self.data.radial_distance;
        self.data
            .frame
    ...
    /// replay its unsquared radical branch at the retained cusp root.
    pub(crate) fn parallel_intersections(
        &self,
        other: &BezierParallel2,
        policy: &CurveContext,
    ) -> CurveResult<Classification<BezierAlgebraicCuspSemicircleParallelIntersections2>> {
        self.parallel_intersections_with_range(other, None, policy)
    }

    pub(crate) fn parallel_intersections_in_range(
        &self,
        other: &BezierParallel2,
        range: &BezierParameterRange2,
        policy: &CurveContext,
    ) -> CurveResult<Classification<BezierAlgebraicCuspSemicircleParallelIntersections2>> {
        self.parallel_intersections_with_range(other, Some(range), policy)
    }

    fn parallel_intersections_with_range(
        &self,
        other: &BezierParallel2,
                    ...
                    BezierAlgebraicCuspSemicircleParallelIntersections2::Overlaps(overlaps) => {
                        BezierAlgebraicCuspSemicircleParallelIntersections2::Overlaps(
                            overlaps
                                .into_iter()
                                .map(|overlap| {
                                    let range = overlap.parallel_range();
                                    BezierAlgebraicCuspSemicircleParallelOverlap2 {
                                        parallel_range: BezierParameterRange2::new_validated(
                                            range.end().unit_complement(),
                                            range.start().unit_complement(),
                                        ),
        ...
        let circle = match reduce_algebraic_cusp_radical_expression(
            system.circle,
            self.cusp_parameter(),
            policy,
        )? {
        ...
        let selected_half_plane = match reduce_algebraic_cusp_radical_expression(
        ...
        let residual_is_rootless = match (diagonal_location, range) {
            (Some(_), Some(range)) => bivariate_fiber_strict_sign_on_parameter_range(
        ...
        let projection = if residual_is_rootless {
            BezierAlgebraicCuspFiberProjection2::Parameters(Vec::new())
        } else {
            match algebraic_cusp_projected_fiber_parameters(
                &projection_incidence,
                self.cusp_parameter(),
                policy,
            )? {
        ...
        let candidates = match projection {
            BezierAlgebraicCuspFiberProjection2::Parameters(parameters) => parameters,
        ...
        // projection roots.
        let mut contacts = diagonal_location
            .map(|location| BezierAlgebraicCuspSemicircleParallelContact2 {
        ...
        for candidate in candidates {
            ...
            let location = if selected_sign == RealSign::Zero {
                ...
                let reduced = match reduce_algebraic_cusp_radical_expression(
                    expression,
                    self.cusp_parameter(),
                    policy,
                )? {
            ...
            let source_cross = match algebraic_cusp_correlated_predicate_sign(
                &contact_incidence,
                &tangent_cross_source,
                &cusp_parameter,
                &candidate,
            ...
            let tangent_cross_sign = if source_cross == RealSign::Zero {
                ...
                other.apply_parallel_derivative_scale_to_cross_sign(
    ...
    fn replay_positive_dimensional_parallel_circle(
        &self,
        range: Option<&BezierParameterRange2>,
        circle: &BezierAlgebraicCuspTwoTermExpression2,
        selected_half_plane: &BezierAlgebraicCuspTwoTermExpression2,
        ...
        let projected = match algebraic_cusp_projected_fiber_parameters(
            &half_incidence,
            self.cusp_parameter(),
            policy,
        )? {
        ...
        let mut selected_zeros = Vec::new();
        for parameter in projected {
            ...
            match (start_order, end_order) {
                (std::cmp::Ordering::Equal, _) => {
                    ...
                    selected_zeros.push(start.clone());
                }
                ...
                (std::cmp::Ordering::Greater, std::cmp::Ordering::Less) => {
                    ...
                    selected_zeros.push(boundary.clone());
        ...
        for pair in boundaries.windows(2) {
            ...
            overlaps.push(BezierAlgebraicCuspSemicircleParallelOverlap2 {
                parallel_range: BezierParameterRange2::new_validated(
        ...
        for boundary in selected_zeros {
    ...
    fn pair_system(
        ...
        other: &Self,
        policy: &CurveContext,
    ) -> CurveResult<Classification<BezierAlgebraicCuspSemicirclePairSystem2>> {
        let first_frame = &self.data.frame.data;
        let second_frame = &other.data.frame.data;
        let (first_center_x, first_center_y) = self
            .data
            .frame
            .point_numerators_at_parallel_distance(first_frame.parallel.distance());
        let (second_center_x, second_center_y) = other
            .data
            .frame
            .point_numerators_at_parallel_distance(second_frame.parallel.distance());
        let delta_x = bivariate_parameter_difference(
            &first_frame.denominator,
            &second_center_x,
            &first_center_x,
            &second_frame.denominator,
        );
        let delta_y = bivariate_parameter_difference(
            &first_frame.denominator,
            &second_center_y,
        ...
        let common_denominator =
            bivariate_outer_product(&first_frame.denominator, &second_frame.denominator);
        let common_denominator_squared =
            bivariate_multiply(&common_denominator, &common_denominator);
        let center_distance_squared = bivariate_add(
            &bivariate_multiply(&delta_x, &delta_x),
            &bivariate_multiply(&delta_y, &delta_y),
        );
        ...
        let second_center_line = bivariate_subtract(
            &center_line,
            &bivariate_scale(center_distance_squared.clone(), &Real::from(2_i8)),
        );
        let discriminant = bivariate_subtract(
            &bivariate_scale(
                bivariate_multiply(&center_distance_squared, &common_denominator_squared),
                &(Real::from(4_i8) * &first_radius_squared),
            ),
            &bivariate_multiply(&center_line, &center_line),
        );

        let unit = [Real::one()];
        let first_denominator = bivariate_outer_product(&first_frame.denominator, &unit);
        let second_denominator = bivariate_outer_product(&unit, &second_frame.denominator);
        let first_normal_dot_delta = bivariate_add(
            &bivariate_multiply_first_parameter(&delta_x, &first_frame.normal_x_numerator),
            &bivariate_multiply_first_parameter(&delta_y, &first_frame.normal_y_numerator),
        );
        let first_normal_cross_delta = bivariate_subtract(
            &bivariate_multiply_first_parameter(&delta_y, &first_frame.normal_x_numerator),
            &bivariate_multiply_first_parameter(&delta_x, &first_frame.normal_y_numerator),
        );
        let second_normal_x = bivariate_outer_product(&unit, &second_frame.normal_x_numerator);
        let second_normal_y = bivariate_outer_product(&unit, &second_frame.normal_y_numerator);
        let second_normal_dot_delta = bivariate_add(
            &bivariate_multiply(&delta_x, &second_normal_x),
            &bivariate_multiply(&delta_y, &second_normal_y),
        );
        let second_normal_cross_delta = bivariate_subtract(
            &bivariate_multiply(&delta_y, &second_normal_x),
            &bivariate_multiply(&delta_x, &second_normal_y),
        );

        let first_half_scale = self.turn_sign() * self.radial_distance();
        let second_half_scale = other.turn_sign() * other.radial_distance();
        let first_selected_half_plane = BezierAlgebraicCuspTwoTermExpression2 {
            rational: bivariate_scale(
                bivariate_multiply(
                    &bivariate_multiply(&center_line, &first_normal_cross_delta),
                    &second_denominator,
                ),
                &first_half_scale,
            ),
            radical: bivariate_scale(
                bivariate_multiply(&first_normal_dot_delta, &second_denominator),
                &first_half_scale,
            ),
        };
        let second_selected_half_plane = BezierAlgebraicCuspTwoTermExpression2 {
            rational: bivariate_scale(
                bivariate_multiply(
                    &bivariate_multiply(&second_center_line, &second_normal_cross_delta),
                    &first_denominator,
                ),
                &second_half_scale,
            ),
            radical: bivariate_scale(
                bivariate_multiply(&second_normal_dot_delta, &first_denominator),
                &second_half_scale,
            ),
        };
        let first_diameter_side = BezierAlgebraicCuspTwoTermExpression2 {
            rational: bivariate_scale(
                bivariate_multiply(
                    &bivariate_multiply(&center_line, &first_normal_dot_delta),
                    &second_denominator,
                ),
                self.radial_distance(),
            ),
            radical: bivariate_scale(
                bivariate_multiply(&first_normal_cross_delta, &second_denominator),
                &(-self.radial_distance().clone()),
            ),
        };
        let second_diameter_side = BezierAlgebraicCuspTwoTermExpression2 {
            rational: bivariate_scale(
                bivariate_multiply(
                    &bivariate_multiply(&second_center_line, &second_normal_dot_delta),
                    &first_denominator,
                ),
                other.radial_distance(),
            ),
            radical: bivariate_scale(
                bivariate_multiply(&second_normal_cross_delta, &first_denominator),
                &(-other.radial_distance().clone()),
            ),
        };
        let radius_denominator_base = bivariate_scale(
            bivariate_multiply(&center_distance_squared, &common_denominator_squared),
            &Real::from(2_i8),
        );
        let system = BezierAlgebraicCuspSemicirclePairSystem2 {
            incidence: BivariatePolynomial::new(vec![
                other.cusp_parameter().polynomial().coefficients().to_vec(),
            ]),
            center_distance_squared,
            discriminant,
            ...
            first_diameter_side,
            second_diameter_side,
            first_radius_squared_denominator: bivariate_scale(
                radius_denominator_base.clone(),
                &first_radius_squared,
            ),
            second_radius_squared_denominator: bivariate_scale(
                radius_denominator_base,
                &second_radius_squared,
            ),
        };
        reduce_algebraic_cusp_pair_system(
            system,
            self.cusp_parameter(),
            other.cusp_parameter(),
            policy,
        )
    }

    fn coincident_pair_system(
        &self,
        other: &Self,
        policy: &CurveContext,
    ) -> CurveResult<Classification<BezierAlgebraicCuspSemicircleCoincidentSystem2>> {
        let first = &self.data.frame.data;
        let second = &other.data.frame.data;
        let normal_dot = bivariate_add(
            &bivariate_outer_product(&first.normal_x_numerator, &second.normal_x_numerator),
            &bivariate_outer_product(&first.normal_y_numerator, &second.normal_y_numerator),
        );
        let normal_cross = bivariate_subtract(
            &bivariate_outer_product(&first.normal_x_numerator, &second.normal_y_numerator),
            &bivariate_outer_product(&first.normal_y_numerator, &second.normal_x_numerator),
        );
        let common_denominator = bivariate_outer_product(&first.denominator, &second.denominator);
        let common_denominator_squared =
            bivariate_multiply(&common_denominator, &common_denominator);
        let radial_scale = self.radial_distance() * other.radial_distance();
        let system = BezierAlgebraicCuspSemicircleCoincidentSystem2 {
            incidence: BivariatePolynomial::new(vec![
                other.cusp_parameter().polynomial().coefficients().to_vec(),
            ]),
            radial_dot: bivariate_scale(
                bivariate_multiply(&normal_dot, &common_denominator),
                &radial_scale,
            ),
            radial_cross: bivariate_scale(
                bivariate_multiply(&normal_cross, &common_denominator),
                &radial_scale,
            ),
            first_radius_squared_denominator: bivariate_scale(
                common_denominator_squared.clone(),
        ...
        reduce_algebraic_cusp_coincident_system(
            system,
            self.cusp_parameter(),
            other.cusp_parameter(),
            policy,
    ...
    fn coincident_endpoint_contacts(
        radial_same: bool,
    ...
    fn coincident_pair_intersections(
        &self,
        other: &Self,
        policy: &CurveContext,
    ) -> CurveResult<Classification<BezierAlgebraicCuspSemicirclePairIntersections2>> {
        let system = match self.coincident_pair_system(other, policy)? {
            Classification::Decided(system) => system,
            Classification::Uncertain(reason) => {
                return Ok(Classification::Uncertain(reason));
            }
        };
        let first_parameter = BezierParameter2::Algebraic(self.cusp_parameter().clone());
        let second_parameter = BezierParameter2::Algebraic(other.cusp_parameter().clone());
        ...
        let radial_cross = match sign(&system.radial_cross)? {
            Classification::Decided(sign) => sign,
            Classification::Uncertain(reason) => {
                return Ok(Classification::Uncertain(reason));
            }
        ...
        let (first_boundaries, second_boundaries) = if radial_cross == RealSign::Zero {
            let radial_dot = match sign(&system.radial_dot)? {
                Classification::Decided(sign) => sign,
                Classification::Uncertain(reason) => {
                    return Ok(Classification::Uncertain(reason));
                }
            };
            let radial_same = match radial_dot {
        ...
        let parameter_map = if radial_cross == RealSign::Zero {
            ...
            BezierAlgebraicCuspSemicirclePairOverlapParameterMapData2::Correlated {
                ...
                radial_cross: system.radial_cross,
                first_radius_squared_denominator: system.first_radius_squared_denominator,
                second_radius_squared_denominator: system.second_radius_squared_denominator,
    ...
    pub(crate) fn pair_intersections(
        ...
        if Arc::ptr_eq(&self.data.frame.data, &other.data.frame.data) {
        ...
        let center_distance_sign = match sign(&system.center_distance_squared)? {
            Classification::Decided(sign) => sign,
            Classification::Uncertain(reason) => {
                return Ok(Classification::Uncertain(reason));
            }
        ...
        match center_distance_sign {
            RealSign::Zero => {
                let radius_difference = self.radial_distance() * self.radial_distance()
                    - other.radial_distance() * other.radial_distance();
                return match real_sign(&radius_difference, policy) {
                    Some(RealSign::Zero) => self.coincident_pair_intersections(other, policy),
            ...
            RealSign::Negative => {
                return Err(CurveError::Topology(
                    "algebraic cusp circle center distance squared was negative".into(),
                ));
            }
        }
        ...
        let radical_sign = |expression: &BezierAlgebraicCuspTwoTermExpression2, branch: i8| {
            let expression = algebraic_cusp_branched_expression(expression, branch);
        ...
        let location = |selected_half_plane: &BezierAlgebraicCuspTwoTermExpression2,
                        diameter_side: &BezierAlgebraicCuspTwoTermExpression2,
                        branch: i8|
         -> CurveResult<
            Classification<Option<BezierAlgebraicCuspSemicircleContactLocation2>>,
        > {
            let selected = match radical_sign(selected_half_plane, branch)? {
                Classification::Decided(sign) => sign,
                Classification::Uncertain(reason) => {
                    return Ok(Classification::Uncertain(reason));
                }
            ...
            Ok(match radical_sign(diameter_side, branch)? {
                ...
                Classification::Decided(RealSign::Zero) => {
                    return Err(CurveError::Topology(
                        "nonzero algebraic semicircle radius had an indeterminate pair endpoint"
                            .into(),
                    ));
                }
                Classification::Uncertain(reason) => Classification::Uncertain(reason),
        ...
        let parameter_map = BezierAlgebraicCuspSemicirclePairParameterMap2 {
            data: Arc::new(BezierAlgebraicCuspSemicirclePairParameterMapData2 {
                ...
                first_diameter_side: system.first_diameter_side,
                second_diameter_side: system.second_diameter_side,
                first_radius_squared_denominator: system.first_radius_squared_denominator,
                second_radius_squared_denominator: system.second_radius_squared_denominator,
                policy: *policy,
            }),
    ...
    > {
        ...
        let candidates = match algebraic_cusp_projected_fiber_parameters(
            &incidence,
            self.cusp_parameter(),
            policy,
        )? {
    ...
    pub(crate) fn contact_order_to_real(
        ...
        let predicate = BezierAlgebraicCuspTwoTermExpression2 {
            rational: bivariate_scaled_difference(
                &self.data.diameter_side.rational,
                &denominator,
                &self.data.radius_squared_denominator,
                &radial_coefficient,
            ),
...
impl BezierAlgebraicCuspSemicirclePairParameterMap2 {
    fn contact_order_to_real_for_side(
        ...
        let predicate = BezierAlgebraicCuspTwoTermExpression2 {
            rational: bivariate_scaled_difference(
                &diameter_side.rational,
                &denominator,
                radius_squared_denominator,
                &radial_coefficient,
            ),
            radical: bivariate_scale(
                diameter_side.radical.clone(),
                &(denominator * Real::from(contact.branch)),
            ),
        };
        Ok(
            match algebraic_cusp_correlated_square_root_sum_sign(
                &self.data.incidence,
                &predicate,
                &self.data.discriminant,
                &self.data.first_cusp_parameter,
                &self.data.second_cusp_parameter,
                policy,
            )? {
                // The normalized diameter coordinate decreases strictly with u.
                Classification::Decided(RealSign::Positive) => {
    ...
    pub(crate) fn first_contact_order_to_real(
        ...
        self.contact_order_to_real_for_side(
            contact,
            parameter,
            contact.first_location,
            &self.data.first_diameter_side,
            &self.data.first_radius_squared_denominator,
        )
    }
    ...
    pub(crate) fn second_contact_order_to_real(
        ...
        self.contact_order_to_real_for_side(
            ...
            &self.data.second_diameter_side,
            &self.data.second_radius_squared_denominator,
        )
    }
...
impl BezierAlgebraicCuspSemicirclePairOverlap2 {
    ...
    pub(crate) fn map_parameter(
        ...
        if let BezierAlgebraicCuspSemicircleParameter2::Mapped(data) = parameter
            ...
            && Arc::ptr_eq(&overlap.data, &self.data)
    ...
    fn mapped_exact_parameter_order_to_real(
        ...
        let source_clockwise = if source_first {
            ...
            *first_clockwise
                ^ (self.data.orientation == RationalBezierOverlapOrientation2::Reversed)
        };
        ...
        let predicate = bivariate_scaled_sum_difference(
            radial_dot,
            &radial_dot_scale,
            radial_cross,
            &radial_cross_scale,
            target_radius_squared_denominator,
            &radius_scale,
        );
        Ok(
            match algebraic_cusp_correlated_predicate_sign(
    ...
    fn mapped_parameter_order_to_real(
        &self,
        source: &BezierAlgebraicCuspSemicircleParameter2,
        source_first: bool,
        target: &Real,
                ...
                Arc::ptr_eq(first, second)
                    || match (first.as_ref(), second.as_ref()) {
                        ...
                        (
                            ...
                            Arc::ptr_eq(&first_overlap.data, &second_overlap.data)
    ...
    pub(crate) fn cmp_by_refinement(
        ...
        loop {
            ...
            refinement_steps = if refinement_steps == 0 {
                ...
                refinement_steps.saturating_mul(2).min(MAX_REFINEMENT_STEPS)
            };
        }
...
fn algebraic_cusp_projected_fiber_parameters(
    incidence: &BivariatePolynomial,
    ...
    let projection = if report.status == CurveIntersectionResultantStatus::DegreeBoundExceeded {
        match algebraic_cusp_quotient_ring_fiber_projection(incidence, &cusp_root, policy)? {
            Classification::Decided(projection) => projection,
            Classification::Uncertain(reason) => {
                return Ok(Classification::Uncertain(reason));
            }
    ...
    cell_boundaries.push(Real::zero());
    ...
    for candidate in candidates {
        let batched_membership = if matches!(&candidate, BezierParameter2::Algebraic(_)) {
            let report = candidate_reports.get(report_index).ok_or_else(|| {
                CurveError::Topology("algebraic cusp candidate report mismatch".into())
            })?;
            report_index += 1;
            if report.certainty == PredicateCertainty::Approximate {
                policy.observe_approximate_512();
        ...
        let belongs = match match batched_membership {
            ...
            None => algebraic_cusp_fiber_contains_parameter(
                incidence,
                &cusp_parameter,
                &candidate,
                policy,
...
fn algebraic_cusp_quotient_ring_fiber_projection(
    incidence: &BivariatePolynomial,
    ...
    let polynomial =
        match BezierParameterPolynomial::try_new_power_basis(report.coefficients, policy)? {
            Classification::Decided(polynomial) => polynomial,
            Classification::Uncertain(reason) => {
                return Ok(Classification::Uncertain(reason));
            }
...
fn algebraic_cusp_fiber_contains_parameter(
    ...
    loop {
        ...
        let (BezierParameter2::Algebraic(cusp), BezierParameter2::Algebraic(other)) = (cusp, other)
            ...
            return algebraic_cusp_fiber_contains_parameter(
                incidence,
        ...
        steps = steps
            .checked_mul(2)
            .and_then(|steps| steps.checked_add(1))
            .ok_or_else(|| CurveError::Topology("algebraic cusp refinement overflow".into()))?;
    }
}

fn algebraic_cusp_correlated_predicate_sign(
    incidence: &BivariatePolynomial,
    ...
    loop {
        ...
        steps = steps
            ...
            .and_then(|steps| steps.checked_add(1))
            .ok_or_else(|| {
                CurveError::Topology("algebraic cusp sign refinement overflow".into())
            })?;
    }
}

fn algebraic_cusp_correlated_radical_sum_sign(
    incidence: &BivariatePolynomial,
...
fn algebraic_cusp_independent_radical_sum_sign(
    expression: &BezierAlgebraicCuspTwoTermExpression2,
    speed_squared: &BivariatePolynomial,
    cusp_parameter: &BezierParameter2,
    other_parameter: &BezierParameter2,
...
fn reduce_algebraic_cusp_pair_system(
    system: BezierAlgebraicCuspSemicirclePairSystem2,
    first: &BezierAlgebraicParameter2,
    second: &BezierAlgebraicParameter2,
    policy: &CurveContext,
) -> CurveResult<Classification<BezierAlgebraicCuspSemicirclePairSystem2>> {
    let BezierAlgebraicCuspSemicirclePairSystem2 {
        incidence,
        center_distance_squared,
        discriminant,
        first_selected_half_plane,
    ...
    macro_rules! reduce_expression {
        ($expression:expr) => {{
            let BezierAlgebraicCuspTwoTermExpression2 { rational, radical } = $expression;
            BezierAlgebraicCuspTwoTermExpression2 {
                rational: reduce!(rational),
                radical: reduce!(radical),
            }
        }};
    }
    Ok(Classification::Decided(
        BezierAlgebraicCuspSemicirclePairSystem2 {
            incidence,
            center_distance_squared: reduce!(center_distance_squared),
            discriminant: reduce!(discriminant),
            first_selected_half_plane: reduce_expression!(first_selected_half_plane),
            second_selected_half_plane: reduce_expression!(second_selected_half_plane),
            first_diameter_side: reduce_expression!(first_diameter_side),
            second_diameter_side: reduce_expression!(second_diameter_side),
            first_radius_squared_denominator: reduce!(first_radius_squared_denominator),
            second_radius_squared_denominator: reduce!(second_radius_squared_denominator),
        },
    ))
}

fn reduce_algebraic_cusp_coincident_system(
    system: BezierAlgebraicCuspSemicircleCoincidentSystem2,
    first: &BezierAlgebraicParameter2,
    second: &BezierAlgebraicParameter2,
    policy: &CurveContext,
) -> CurveResult<Classification<BezierAlgebraicCuspSemicircleCoincidentSystem2>> {
    let BezierAlgebraicCuspSemicircleCoincidentSystem2 {
        incidence,
        radial_dot,
        radial_cross,
        first_radius_squared_denominator,
    ...
    Ok(Classification::Decided(
        BezierAlgebraicCuspSemicircleCoincidentSystem2 {
            incidence,
            radial_dot: reduce!(radial_dot),
            radial_cross: reduce!(radial_cross),
            first_radius_squared_denominator: reduce!(first_radius_squared_denominator),
            second_radius_squared_denominator: reduce!(second_radius_squared_denominator),
        },
    ))
...
fn bivariate_fiber_strict_sign_on_parameter_range(
    polynomial: &BivariatePolynomial,
    retained: &BezierAlgebraicParameter2,
    fiber_range: &BezierParameterRange2,
    policy: &CurveContext,
) -> CurveResult<Option<RealSign>> {
    let retained = BezierParameter2::Algebraic(retained.clone());
    fn bounds(parameter: &BezierParameter2) -> (&Real, &Real) {
        match parameter {
    ...
    for refinement_steps in [0, 1, 2, 4, 8, 16] {
        let retained = retained
            .clone()
            .refined_isolating_interval(refinement_steps, policy);
        let fiber_start = fiber_range
            .start()
            .clone()
            .refined_isolating_interval(refinement_steps, policy);
        let fiber_end = fiber_range
            .end()
            .clone()
            .refined_isolating_interval(refinement_steps, policy);
        let (retained_start, retained_end) = bounds(&retained);
        let (start_lower, start_upper) = bounds(&fiber_start);
        let (end_lower, end_upper) = bounds(&fiber_end);
        let fiber_lower = match compare_reals(start_lower, end_lower, policy) {
            Some(std::cmp::Ordering::Less | std::cmp::Ordering::Equal) => start_lower,
            Some(std::cmp::Ordering::Greater) => end_lower,
            None => continue,
        };
        let fiber_upper = match compare_reals(start_upper, end_upper, policy) {
            Some(std::cmp::Ordering::Greater | std::cmp::Ordering::Equal) => start_upper,
            Some(std::cmp::Ordering::Less) => end_upper,
            None => continue,
        };
    ...
    // A rectangular enclosure can remain inconclusive when the polynomial
    // changes sign at nearby retained-parameter values.  Specialize only its
    // Bernstein controls to the exact algebraic fiber before escalating to a
    // resultant.  Uniform strict control signs prove the entire fiber range
    // root-free while avoiding a Sturm sequence in Q(alpha).
    let fiber_start = fiber_range
        .start()
        .clone()
        .refined_isolating_interval(16, policy);
    let fiber_end = fiber_range
        .end()
        .clone()
        .refined_isolating_interval(16, policy);
    ...
    let retained = match &retained {
        BezierParameter2::Algebraic(retained) => retained,
        BezierParameter2::Exact(_) => unreachable!("retained cusp parameter is algebraic"),
    };
    ...
    while let Some((controls, depth)) = pending.pop() {
        ...
        let endpoints_are_positive =
            signs.first() == Some(&RealSign::Positive) && signs.last() == Some(&RealSign::Positive);
        let endpoints_are_negative =
            signs.first() == Some(&RealSign::Negative) && signs.last() == Some(&RealSign::Negative);
...
fn bivariate_scaled_sum_difference(
    first: &BivariatePolynomial,
    first_scale: &Real,
    second: &BivariatePolynomial,
    second_scale: &Real,
    subtrahend: &BivariatePolynomial,
    subtrahend_scale: &Real,
) -> BivariatePolynomial {
    let first_count = first
        .coefficients
        .len()
        .max(second.coefficients.len())
        .max(subtrahend.coefficients.len());
    let second_count = first
        .coefficients
        .iter()
        .chain(&second.coefficients)
        .chain(&subtrahend.coefficients)
        .map(Vec::len)
        .max()
        .unwrap_or(0);
    let mut coefficients = vec![vec![Real::zero(); second_count]; first_count];
    ...
    for (target, source) in coefficients.iter_mut().zip(&subtrahend.coefficients) {
        for (target, source) in target.iter_mut().zip(source) {
            *target -= source * subtrahend_scale;
        }
    }
    BivariatePolynomial::new(coefficients)
}
...
mod conversion_tests {
    ...
    fn rational_tangent_coordinates(
        ...
        if let Some((dx_numerator, dy_numerator, denominator)) =
            image.retained_coordinate_polynomials()
        {
            return [(dx_numerator, denominator), (dy_numerator, denominator)];
        }
        let dx = image.dx().expect("represented dx image");
        let dy = image.dy().expect("represented dy image");
        [
            (dx.numerator_coefficients(), dx.denominator_coefficients()),
            (dy.numerator_coefficients(), dy.denominator_coefficients()),
        ]
    }
    ...
    fn assert_rational_radius_tangent_relation(
        ...
        parameter: &BezierParameter2,
        expected_cross_sign: RealSign,
        policy: &CurveContext,
    ) {
        let point = rational_image_coordinates(point);
        let center = rational_image_coordinates(center);
        let tangent = rational_tangent_coordinates(tangent);
        let radius = [
            rational_coordinate_difference(point[0], center[0]),
            rational_coordinate_difference(point[1], center[1]),
        ];

        let dot = polynomial_add(
            &polynomial_multiply(
                &polynomial_multiply(&radius[0].0, tangent[0].0),
                &polynomial_multiply(&radius[1].1, tangent[1].1),
            ),
            &polynomial_multiply(
                &polynomial_multiply(&radius[1].0, tangent[1].0),
                &polynomial_multiply(&radius[0].1, tangent[0].1),
            ),
        );
        assert_algebraic_polynomial_sign(dot, parameter, RealSign::Zero, policy);

        let common_denominator = polynomial_multiply(
            &polynomial_multiply(&radius[0].1, &radius[1].1),
            &polynomial_multiply(tangent[0].1, tangent[1].1),
        );
        let cross_numerator = polynomial_subtract(
            &polynomial_multiply(
                &polynomial_multiply(&radius[0].0, tangent[1].0),
                &polynomial_multiply(&radius[1].1, tangent[0].1),
            ),
            &polynomial_multiply(
                &polynomial_multiply(&radius[1].0, tangent[0].0),
                &polynomial_multiply(&radius[0].1, tangent[1].1),
            ),
        );
        assert_algebraic_polynomial_sign(
            polynomial_multiply(&cross_numerator, &common_denominator),
            parameter,
            expected_cross_sign,
            policy,
        );
    ...
    fn general_algebraic_cusp_semicircle(
        ...
        let parallel = CubicBezier2::new(
            ...
            Point2::new(Real::zero(), Real::from(4_i8)),
            Point2::new(Real::from(4_i8), Real::from(-4_i8)),
            Point2::new(Real::from(4_i8), Real::zero()),
        )
        .parallel_left(half)
        .unwrap();
        let Classification::Decided(analysis) = parallel.singularity_analysis(policy).unwrap()
        else {
            panic!("general cusp analysis must be decided");
        };
        let BezierParameter2::Algebraic(parameter) = &analysis.parallel_cusps()[0] else {
            panic!("selected general cusp must be algebraic");
        };
        let Classification::Decided(Some(semicircle)) = parallel
            .algebraic_cusp_semicircle(parameter, radial_distance, clockwise, policy)
            .unwrap()
        else {
            panic!("certified cusp semicircle must be constructed");
        };
    ...
    fn general_algebraic_cusp_semicircle_is_exact_shared_and_oriented() {
        ...
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            let (_parallel, parameter, semicircle) =
                general_algebraic_cusp_semicircle(quarter.clone(), true, &policy);
            assert_eq!(
                std::mem::size_of::<BezierAlgebraicCuspSemicircle2>(),
                std::mem::size_of::<usize>()
            );
            assert!(semicircle.is_clockwise());
            assert_eq!(semicircle.radial_distance(), &quarter);
            assert_eq!(semicircle.cusp_parameter(), &parameter);
            assert!(parameter_representation(&parameter, &policy).is_valid());

            let zero = Real::zero();
            let one = Real::one();
            let half = (Real::one() / Real::from(2_i8)).unwrap();
            let one_quarter = quarter.clone();
            let three_quarters = Real::one() - &one_quarter;
            let Classification::Decided(start) = semicircle.point_at(&zero, &policy).unwrap()
            else {
                panic!("semicircle start must be decided");
            };
            let Classification::Decided(end) = semicircle.point_at(&one, &policy).unwrap() else {
                panic!("semicircle end must be decided");
            };
            assert_eq!(start, semicircle.start_point_image(&policy).unwrap());
            assert_eq!(end, semicircle.end_point_image(&policy).unwrap());
            let center = semicircle.center_point_image(&policy).unwrap();
            for curve_parameter in [zero, one_quarter.clone(), half, three_quarters, one] {
                let Classification::Decided(point) =
                    semicircle.point_at(&curve_parameter, &policy).unwrap()
                else {
                    panic!("semicircle point must be decided");
                };
                let Classification::Decided(tangent) =
                    semicircle.tangent_at(&curve_parameter, &policy).unwrap()
                else {
                    panic!("semicircle tangent must be decided");
                };
                ...
                assert_rational_radius_tangent_relation(
                    &point,
                    &center,
                    &tangent,
                    &BezierParameter2::Algebraic(parameter.clone()),
            ...
            let Classification::Decided(forward_point) =
                semicircle.point_at(&one_quarter, &policy).unwrap()
            else {
                panic!("forward point must be decided");
            };
            let Classification::Decided(reverse_point) = reversed
                .point_at(&(Real::one() - &one_quarter), &policy)
                .unwrap()
            else {
                panic!("reverse point must be decided");
            };
            for (forward, reverse) in rational_image_coordinates(&forward_point)
                .into_iter()
                .zip(rational_image_coordinates(&reverse_point))
            {
                let (numerator, _) = rational_coordinate_difference(forward, reverse);
                assert_algebraic_polynomial_sign(
                    numerator,
                    &BezierParameter2::Algebraic(parameter.clone()),
                    RealSign::Zero,
    ...
    fn algebraic_cusp_rational_parameter_map_brackets_an_interior_contact() {
        ...
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            ...
            let retained_parameter = map.contact_parameter(contact);
            assert!(matches!(
                &retained_parameter,
                BezierAlgebraicCuspSemicircleParameter2::Mapped(_)
            ));
            let other_policy = if policy == CurveContext::STRICT {
            ...
            assert_eq!(
                retained_parameter.order_to_real(&half, &policy).unwrap(),
                Classification::Decided(std::cmp::Ordering::Less),
            );
    ...
    fn non_ph_parallel_touching_upper_unit_circle(at_top: bool) -> BezierParallel2 {
        let quarter = (Real::one() / Real::from(4_i8)).unwrap();
        let half = (Real::one() / Real::from(2_i8)).unwrap();
        let five_quarters = Real::one() + &quarter;
        let nine_quarters = Real::from(2_i8) + &quarter;
        let source = if at_top {
            QuadraticBezier2::new(
                Point2::new(Real::zero(), five_quarters.clone()),
                Point2::new(half, five_quarters),
                Point2::new(Real::one(), nine_quarters),
            )
        } else {
            QuadraticBezier2::new(
                Point2::new(Real::one(), quarter.clone()),
                Point2::new((Real::from(3_i8) / Real::from(2_i8)).unwrap(), quarter),
                Point2::new(Real::from(2_i8), five_quarters),
            )
        };
        source
    ...
    fn algebraic_cusp_semicircle_intersects_a_general_analytic_parallel_exactly() {
        ...
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            let semicircle = synthetic_reducible_cusp_semicircle((3, 4), ((2, 3), (4, 5)), &policy);
            let parallel = non_ph_parallel_touching_upper_unit_circle(true);
            let Classification::Decided(
                BezierAlgebraicCuspSemicircleParallelIntersections2::Contacts(contacts),
            ) = semicircle
                ...
                panic!("algebraic cusp/general-parallel contacts must complete");
            };
            let [contact] = contacts.as_slice() else {
                panic!("the tangent fixture must have one selected contact");
            };
            assert_eq!(
                contact
                    .parallel_parameter
            ...
            let Classification::Decided(map) = semicircle
                ...
                panic!("analytic-parallel contact parameter map must complete");
            };
            assert_eq!(
                std::mem::size_of::<BezierAlgebraicCuspSemicircleParallelParameterMap2>(),
                std::mem::size_of::<usize>(),
            );
            let clone = map.clone();
            assert!(Arc::ptr_eq(&map.data, &clone.data));
            ...
            assert_eq!(
                retained_parameter
                    .order_to_real(&three_quarters, &policy)
                    .unwrap(),
                Classification::Decided(std::cmp::Ordering::Less),
            );
            assert_eq!(
            ...
            // Squared incidence also vanishes at t=0 on the opposite normal
            // branch, whose actual parallel lies outside the circle.
            let opposite_branch = parallel.with_distance(quarter.clone());
            assert_eq!(
                semicircle
                    .parallel_intersections(&opposite_branch, &policy)
                    .unwrap(),
                Classification::Decided(
                    BezierAlgebraicCuspSemicircleParallelIntersections2::Contacts(Vec::new())
                ),
    ...
    fn algebraic_cusp_semicircle_replays_a_selected_circle_component() {
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            ...
            for parameter in [Real::zero(), Real::one()] {
                ...
                assert_eq!(
                    semicircle.contains_point(&point, &policy).unwrap(),
                    Classification::Decided(true),
                );
            }
            let Classification::Decided(
                ...
                panic!("the selected positive-dimensional circle must publish overlap evidence");
            };
            let [overlap] = overlaps.as_slice() else {
                panic!("the quarter circle must produce one selected overlap");
            };
            assert_eq!(
                overlap.parallel_range(),
                &BezierParameterRange2::new_validated(
                    BezierParameter2::Exact(Real::zero()),
                    BezierParameter2::Exact(Real::one()),
                ),
            ...
            assert_eq!(
                overlap
                    .cusp_end_parameter()
                    .order_to_real(&(Real::one() / Real::from(2_i8)).unwrap(), &policy,)
                    .unwrap(),
                Classification::Decided(std::cmp::Ordering::Equal),
            );
            ...
            let Classification::Decided(
                ...
                panic!("an authored subrange of the circle component must remain exact");
            };
            let [partial_overlap] = partial_overlaps.as_slice() else {
                panic!("the selected partial quarter circle must remain one overlap");
            };
            assert_eq!(partial_overlap.parallel_range(), &partial_range);
            assert_eq!(
            ...
            assert!(reversed_parallel.data.certified_ph_offset.set(None).is_ok());
            let Classification::Decided(
                ...
                panic!("reversing the analytic source must preserve its exact overlap");
            };
            let [reversed_overlap] = reversed_overlaps.as_slice() else {
                panic!("the reversed quarter circle must remain one overlap");
            };
            assert_eq!(
                reversed_overlap.parallel_range(),
                &BezierParameterRange2::new_validated(
                    BezierParameter2::Exact(Real::zero()),
                    BezierParameter2::Exact(Real::one()),
                ),
            ...
            let cusp_start = semicircle
                ...
                .expect("the synthetic cusp start must be represented");
            let cusp_end = semicircle
                .end_point_image(&policy)
                .unwrap()
                .exact_rational_point(&policy)
                .expect("the synthetic cusp end must be represented");
            ...
            let cusp_boundary = CurveRegionBoundaryLoop2::new(
                vec![
            ...
            let Classification::Decided(parallel_fragment) =
                BezierParallelFragment2::try_new(parallel.clone(), source_range, &policy).unwrap()
            else {
            ...
            let parallel_boundary = CurveRegionBoundaryLoop2::new(
                vec![
                    BezierSplitFragment2::AnalyticParallel(parallel_fragment),
                    line_fragment(parallel_end, Point2::new(Real::zero(), Real::zero())),
                    line_fragment(Point2::new(Real::zero(), Real::zero()), parallel_start),
                ],
                &policy,
            )
            .unwrap();
            let parallel_region = CurveRegion2::try_new_with_loop_topology(
                vec![parallel_boundary],
                vec![CurveRegionLoopRole::Material],
                vec![FillRule::NonZero],
                vec![CurveBoundaryInteriorSide2::Left],
            )
            ...
            assert_eq!(
                semicircle
                    .parallel_intersections(&opposite, &policy)
                    .unwrap(),
                Classification::Decided(
                    BezierAlgebraicCuspSemicircleParallelIntersections2::Contacts(Vec::new()),
                ),
    ...
    fn algebraic_cusp_semicircle_parallel_endpoint_respects_reversal() {
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            let semicircle = synthetic_reducible_cusp_semicircle((3, 4), ((2, 3), (4, 5)), &policy);
            let parallel = non_ph_parallel_touching_upper_unit_circle(false);
            for (carrier, expected_location, expected_cross) in [
                (
                    semicircle.clone(),
                    BezierAlgebraicCuspSemicircleContactLocation2::Start,
                    RealSign::Negative,
                ...
                let Classification::Decided(
                    BezierAlgebraicCuspSemicircleParallelIntersections2::Contacts(contacts),
                ) = carrier.parallel_intersections(&parallel, &policy).unwrap()
                else {
                    panic!("algebraic cusp/general-parallel endpoint must complete");
                };
                let contact = contacts
                    ...
                    .expect("the constructed endpoint contact must be retained");
                assert_eq!(contact.location, expected_location);
                assert_eq!(contact.tangent_cross_sign, Some(expected_cross));
            }
        }
    }
    ...
    fn algebraic_cusp_semicircle_pair_replays_independent_selected_roots() {
        ...
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            ...
            let second = synthetic_reducible_cusp_semicircle((1, 4), ((1, 5), (1, 3)), &policy)
                .transform_similarity(&translate_second)
                .unwrap();
            let Classification::Decided(
                BezierAlgebraicCuspSemicirclePairIntersections2::Contacts {
                    contacts,
                ...
                panic!("independent algebraic cusp-circle pair must complete");
            };
            let [contact] = contacts.as_slice() else {
                panic!("the two selected upper unit semicircles share one contact");
            };
            assert_eq!(contact.branch, 1);
            assert_eq!(
                contact.first_location,
                BezierAlgebraicCuspSemicircleContactLocation2::Interior,
            );
            ...
            assert_eq!(
                std::mem::size_of::<BezierAlgebraicCuspSemicirclePairParameterMap2>(),
                std::mem::size_of::<usize>(),
            );
            let clone = map.clone();
            assert!(Arc::ptr_eq(&map.data, &clone.data));
            let first_parameter = map.first_contact_parameter(contact);
            let second_parameter = map.second_contact_parameter(contact);
            assert_eq!(
            ...
            ) = second.pair_intersections(&first, &policy).unwrap()
            else {
                panic!("swapped algebraic cusp-circle pair must complete");
            };
            let [swapped] = swapped.as_slice() else {
                panic!("swapped pair must retain the same unique contact");
            };
            assert_eq!(swapped.branch, -1);
            assert_eq!(swapped.tangent_cross_sign, RealSign::Negative);
            assert_eq!(swapped.first_location, contact.second_location);
            assert_eq!(swapped.second_location, contact.first_location);
        }
    }
    ...
    fn algebraic_cusp_semicircle_pair_handles_tangent_disjoint_and_coincident_supports() {
        let transform = |translation_x, reflect_y| {
            Similarity2::try_from_real_affine(
                ...
                Real::zero(),
                Real::zero(),
                Real::from(if reflect_y { -1_i8 } else { 1_i8 }),
        ...
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            ...
            let tangent = foreign()
                .transform_similarity(&transform(7, false))
                .unwrap();
            let Classification::Decided(
                BezierAlgebraicCuspSemicirclePairIntersections2::Contacts {
                    contacts,
            ...
            ) = first.pair_intersections(&tangent, &policy).unwrap()
            else {
                panic!("tangent algebraic cusp-circle pair must complete");
            };
            let [contact] = contacts.as_slice() else {
                panic!("externally tangent unit circles must have one contact");
            };
            ...
            let disjoint = foreign()
                .transform_similarity(&transform(8, false))
                .unwrap();
            assert!(matches!(
                first.pair_intersections(&disjoint, &policy).unwrap(),
                Classification::Decided(
                    BezierAlgebraicCuspSemicirclePairIntersections2::NoContacts
                ),
            ));
            let opposite_half = foreign().transform_similarity(&transform(6, true)).unwrap();
            assert!(matches!(
                first.pair_intersections(&opposite_half, &policy).unwrap(),
                Classification::Decided(
                    BezierAlgebraicCuspSemicirclePairIntersections2::NoContacts
                ),
            ));
            assert!(matches!(
                first
                    .pair_intersections(&first.reversed(), &policy)
                    .unwrap(),
    ...
    fn algebraic_cusp_semicircle_pair_maps_full_partial_and_endpoint_only_overlap() {
        ...
        .unwrap();
        let lower_same_radial = Similarity2::try_from_real_affine(
            Real::one(),
            Real::zero(),
            Real::zero(),
            Real::from(-1_i8),
        ...
        .unwrap();
        let lower_opposite_radial = Similarity2::try_from_real_affine(
            Real::from(-1_i8),
            Real::zero(),
            Real::zero(),
            Real::from(-1_i8),
        ...
        let oblique_to_origin = Similarity2::try_from_real_affine(
            three_fifths.clone(),
            -four_fifths.clone(),
            four_fifths.clone(),
            three_fifths.clone(),
        ...
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            ...
            for (second, expected_orientation) in [
                ...
                let Classification::Decided(
                    ...
                    panic!("equal selected half circles must overlap completely");
                };
                ...
                assert_eq!(
                    std::mem::size_of::<BezierAlgebraicCuspSemicirclePairOverlap2>(),
                    std::mem::size_of::<usize>(),
                );
                assert_eq!(
                ...
                let full = BezierAlgebraicCuspSemicircleFragment2::full(first.clone(), &policy);
                assert_eq!(
                    std::mem::size_of::<BezierAlgebraicCuspSemicircleFragment2>(),
                    std::mem::size_of::<usize>(),
                );
                assert_eq!(
                    full.representative_parameter().unwrap(),
                    Classification::Decided(half.clone()),
                );
            }
            ...
            let Classification::Decided(BezierAlgebraicCuspSemicirclePairIntersections2::Overlap(
                overlap,
            )) = first.pair_intersections(&partial, &policy).unwrap()
            else {
            ...
            assert_eq!(
                mapped_quadrant_parameter
                    .order_to_real(&third, &policy)
                    .unwrap(),
                Classification::Decided(std::cmp::Ordering::Equal),
            );
            assert_eq!(
                overlap
                    .map_parameter(&mapped_quadrant_parameter, false)
                    .cmp_by_refinement(
                        &BezierAlgebraicCuspSemicircleParameter2::Exact(three_quarters.clone()),
                        &policy,
                    )
            ...
            let Classification::Decided(first_fragment) =
                BezierAlgebraicCuspSemicircleFragment2::try_new(
                    first.clone(),
                    BezierAlgebraicCuspSemicircleParameter2::Exact(three_fifths.clone()),
                    BezierAlgebraicCuspSemicircleParameter2::Exact(four_fifths.clone()),
                    false,
                    &policy,
                )
                .unwrap()
            else {
                panic!("the represented first cusp subfragment must be ordered");
            };
            ...
            for (actual, expected) in [
                ...
                (second_clipped.start(), &quarter),
                (second_clipped.end(), &three_eighths),
            ] {
                assert_eq!(
                    actual
                        .as_algebraic_cusp()
                        .expect("a clipped cusp range must retain cusp parameters")
            ...
            let Classification::Decided(fragment) =
                ...
                panic!("the exact overlap subfragment must have ordered cuts");
            };
            assert!(!fragment.is_reversed());
            assert_eq!(fragment.semicircle(), &first);
            assert_eq!(
                fragment
                    .start_parameter()
                    .cmp_by_refinement(
            ...
            assert_eq!(
                fragment.representative_parameter().unwrap(),
                Classification::Decided(three_quarters.clone()),
            );
            assert_eq!(
                fragment.representative_point().unwrap(),
                first.point_at(&three_quarters, &policy).unwrap(),
            );
            let reversed_fragment = fragment.reversed();
            assert!(reversed_fragment.is_reversed());
            assert_eq!(
                reversed_fragment
                    .start_parameter()
                    .cmp_by_refinement(fragment.start_parameter(), &policy)
                    .unwrap(),
                Classification::Decided(std::cmp::Ordering::Equal),
            );
            assert_eq!(
            ...
            let Classification::Decided(BezierAlgebraicCuspSemicirclePairIntersections2::Overlap(
                oblique_overlap,
            )) = first.pair_intersections(&oblique, &policy).unwrap()
            else {
                panic!("oblique coincident support must retain its partial overlap");
            };
            assert_eq!(
                oblique_overlap.first_boundaries(),
                [
                    BezierAlgebraicCuspSemicirclePairEndpoint2::SecondStart,
                    BezierAlgebraicCuspSemicirclePairEndpoint2::FirstEnd,
                ],
            ...
            let oblique_start = oblique_overlap.first_start_parameter();
            assert!(matches!(
                &oblique_start,
                BezierAlgebraicCuspSemicircleParameter2::Mapped(_)
            ));
            assert_eq!(
                oblique_start
                    .cmp_by_refinement(&oblique_overlap.first_start_parameter(), &policy)
                    .unwrap(),
                Classification::Decided(std::cmp::Ordering::Equal),
            );
            ...
            let Classification::Decided(BezierAlgebraicCuspSemicirclePairIntersections2::Overlap(
                swapped_overlap,
            )) = partial.pair_intersections(&first, &policy).unwrap()
            else {
                panic!("swapped partial overlap must retain the same quadrant");
            };
            assert_eq!(
                swapped_overlap.first_boundaries(),
                [
                    BezierAlgebraicCuspSemicirclePairEndpoint2::FirstStart,
                    BezierAlgebraicCuspSemicirclePairEndpoint2::SecondEnd,
                ],
            ...
            let reversed_partial = partial.reversed();
            let Classification::Decided(BezierAlgebraicCuspSemicirclePairIntersections2::Overlap(
                overlap,
            )) = first
                .pair_intersections(&reversed_partial, &policy)
                .unwrap()
            else {
            ...
            for (second, expected_pairs) in [
                (
                    first.transform_similarity(&lower_same_radial).unwrap(),
                    [
                        (
                            BezierAlgebraicCuspSemicircleContactLocation2::Start,
                            BezierAlgebraicCuspSemicircleContactLocation2::Start,
                ...
                (
                    foreign()
                        .transform_similarity(&lower_opposite_radial)
                        .unwrap(),
                    [
                        (
                            BezierAlgebraicCuspSemicircleContactLocation2::Start,
                ...
                let Classification::Decided(
                    ...
                    panic!("complementary selected semicircles must share both endpoints");
                };
                assert_eq!(contacts.len(), 2);
                for (contact, expected) in contacts.iter().zip(expected_pairs) {
                    assert_eq!((contact.first_location, contact.second_location), expected,);
                    assert_eq!(contact.tangent_cross_sign, RealSign::Zero);
                }
            }
            ...
            assert!(matches!(
                first.pair_intersections(&larger, &policy).unwrap(),
                Classification::Decided(
                    BezierAlgebraicCuspSemicirclePairIntersections2::NoContacts
                ),
            ));
    ...
    fn algebraic_cusp_circle_rational_system_replays_shared_source_endpoint() {
        let half = (Real::one() / Real::from(2_i8)).unwrap();
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            let (parallel, parameter, semicircle) =
                general_algebraic_cusp_semicircle(half.clone(), false, &policy);
            let source = parallel.source().to_rational_bezier().unwrap();
            let system = semicircle.rational_system(&source).unwrap();
            let parameter = BezierParameter2::Algebraic(parameter);
            assert_eq!(
                signed_bivariate_at_parameter_pair(
                    &system.incidence,
                    &parameter,
            ...
            let Classification::Decided(
                ...
                panic!("the algebraic semicircle/source contacts must complete: {intersections:?}");
            };
            let cusp_contact = contacts
                .iter()
                .find(|contact| {
                    matches!(
                        contact.other_parameter.same_value(&parameter, &policy),
                        Ok(Classification::Decided(true))
                    )
                })
    ...
    fn synthetic_reducible_cusp_semicircle(
        ...
        let source = QuadraticBezier2::from_line_segment(
            LineSeg2::try_new(
                Point2::new(Real::from(-5_i8), Real::zero()),
                Point2::new(Real::zero(), Real::zero()),
            )
            .unwrap(),
        );
    ...
    fn curve_region_boolean_clips_correlated_partial_cusp_overlaps() {
        ...
        let first_start = rational(3, 5);
        let first_end = rational(4, 5);
        let second_start = rational(1, 4);
        let second_end = rational(1, 2);
        let expected = [
            rational(2, 3),
            first_end.clone(),
            second_start.clone(),
        ...
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            ...
            let second = synthetic_reducible_cusp_semicircle((1, 4), ((1, 5), (1, 3)), &policy)
                .transform_similarity(&quarter_turn_to_origin)
                .unwrap();
            let segment_region = |semicircle: BezierAlgebraicCuspSemicircle2,
                                  start: Real,
                                  end: Real| {
                let Classification::Decided(fragment) =
                    BezierAlgebraicCuspSemicircleFragment2::try_new(
                    ...
                    panic!("the represented circular-segment arc must be ordered");
                };
                let arc = crate::BezierSplitFragment2::AlgebraicCuspSemicircle(fragment.clone());
                let start = fragment
                    .endpoint_exact_point(true, &policy)
                    .unwrap()
                    .expect("the synthetic segment start must be rational");
                let end = fragment
                    .endpoint_exact_point(false, &policy)
                    .unwrap()
                    .expect("the synthetic segment end must be rational");
                let chord = crate::BezierSplitFragment2::Materialized {
                    start: BezierParameter2::Exact(Real::zero()),
                    end: BezierParameter2::Exact(Real::one()),
                    curve: crate::BezierSubcurve2::Quadratic(QuadraticBezier2::from_line_segment(
                        LineSeg2::try_new(end, start).unwrap(),
            ...
            let first_region = segment_region(first, first_start.clone(), first_end.clone());
            ...
            for (actual, expected) in [
                ...
                assert_eq!(
                    actual
                        .as_algebraic_cusp()
                        .expect("the shared subarc must retain cusp parameters")
                        .order_to_real(expected, &policy)
                        .unwrap(),
                    Classification::Decided(std::cmp::Ordering::Equal),
                );
    ...
    fn simple_general_algebraic_cusp_loop_regularizes_without_materializing_its_field() {
        let quarter = (Real::one() / Real::from(4_i8)).unwrap();
        for policy in [CurveContext::STRICT, CurveContext::APPROXIMATE_512] {
            let (_parallel, source_parameter, semicircle) =
                general_algebraic_cusp_semicircle(quarter.clone(), false, &policy);
            let cusp_fragment =
                BezierAlgebraicCuspSemicircleFragment2::full(semicircle.clone(), &policy);
            let Classification::Decided(representative) =
                cusp_fragment.representative_point().unwrap()
            else {
                panic!("the general cusp representative expression must be retained");
            };
            assert!(
                representative.exact_rational_point(&policy).is_none(),
                "the topology proof must not depend on a rationalized cusp field"
            );

            let source_parameter = BezierParameter2::Algebraic(source_parameter);
            ...
            let regular_span = |parallel: &BezierParallel2| {
                ...
                match crate::BezierParallelFragment2::try_new(parallel.clone(), range, &policy)
                    .unwrap()
                {
            ...
            for (parallel, fragment, expected_location) in [
                (
                    &start_parallel,
                    &start_fragment,
                    BezierAlgebraicCuspSemicircleContactLocation2::Start,
                ...
                let classified = semicircle
                    .parallel_intersections_in_range(parallel, fragment.range(), &policy)
                    .unwrap();
                let Classification::Decided(intersections) = classified else {
                    panic!(
                        "adjacent cusp/parallel range intersection at {expected_location:?} must decide: {classified:?}"
                    );
                };
                let BezierAlgebraicCuspSemicircleParallelIntersections2::Contacts(contacts) =
                    intersections
                else {
                    panic!("adjacent cusp/parallel range intersection: {intersections:?}");
                };
                assert!(
                    contacts.iter().any(|contact| {
                        contact.location == expected_location
                            && contact
                                .parallel_parameter
                                .same_value(&source_parameter, &policy)
                                == Ok(Classification::Decided(true))
                    }),
                    "adjacent contacts: {contacts:?}"
                );
            }
            let reversed_start = start_parallel.reversed();
            let reversed_range = BezierParameterRange2::new_validated(
                start_fragment.range().end().unit_complement(),
                start_fragment.range().start().unit_complement(),
            );
            let Classification::Decided(
                BezierAlgebraicCuspSemicircleParallelIntersections2::Contacts(contacts),
            ) = semicircle
                .parallel_intersections_in_range(&reversed_start, &reversed_range, &policy)
                .unwrap()
            else {
                panic!("reversed adjacent cusp/parallel range intersection must decide");
            };
            let reversed_source_parameter = source_parameter.unit_complement();
            assert!(contacts.iter().any(|contact| {
                contact.location == BezierAlgebraicCuspSemicircleContactLocation2::Start
                    ...
                    && contact
                        .parallel_parameter
                        .same_value(&reversed_source_parameter, &policy)
                        == Ok(Classification::Decided(true))
            }));
            ...
            let Classification::Decided(
                BezierAlgebraicCuspSemicirclePairIntersections2::EndpointContacts(contacts),
            ) = semicircle.pair_intersections(&complement, &policy).unwrap()
            else {
                panic!("complementary general cusp halves must meet only at their endpoints");
            };
            assert_eq!(contacts.len(), 2);
            let fragments = vec![
                crate::BezierSplitFragment2::AlgebraicCuspSemicircle(cusp_fragment),
                crate::BezierSplitFragment2::AlgebraicCuspSemicircle(
                    BezierAlgebraicCuspSemicircleFragment2::full(complement, &policy),
                ),
            ];
            let boundary = crate::CurveRegionBoundaryLoop2::new(fragments, &policy)
                .expect("the correlated cusp endpoints must close the authored loop");
